...
function parser(str, config) {
  ...
  return function match(text) {
    ...
    if (exp && exp.fn) {
      const r = exp.fn(m, config);
      if (!r) {
        return match(text.substring(m.index + m[0].length));
      }
      return r;
    }
    return m[1] || m[0];
...
const DEFAULT_EXPRESSIONS = {
  ...
  DATE: expression(/((?:\d{1,4}[\.\/\-]\d{1,2}[\.\/\-]\d{2,4})|(?:\d{1,2}[\.\/ ]{1,2}\w{3,}[\.\/ ]{1,2}\d{2,4})|(?:\w{3,}[\. ]{1,2}\d{1,2}[\., ]{1,3}\d{2,4}))/, (_) => {
    if (!_) {
      return null;
    }
